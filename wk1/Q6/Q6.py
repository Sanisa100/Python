...
class Pet:
    def __str__(petObj):
        return '{0}: {1}'.format(petObj.animal, petObj.nickname)

    def __init__(petObj, animal, nickname):
        petObj.animal = animal
        petObj.nickname = nickname
...
class PetLinkedList():
    ...
    def __str__(self):
        # {
        """
        Returns MyLinkedList[firstNodeDatum -> nodeDatum -> nodeDatum...]
        """
        # start at the first node, travel down the list
        # and construct datum string
        firstNodeFlag = True
        node = self.firstNode
        datumString = ""
        ...
        while node != None:
            # {
            if firstNodeFlag:
                datumString = datumString + str(node.datum)
            else:
                datumString = datumString + ", " + str(node.datum)
        ...
        return "Pets[{}]".format(datumString)
    # }
# }



linkedListObj = PetLinkedList()
linkedListObj.insertFirst(Pet("dog", "Baxter"))
linkedListObj.insertFirst(Pet("cat", "Kitty"))
linkedListObj.insertFirst(Pet("frog", "Frogory"))
